...
export default function ShopPage() {
  ...
  const getFilteredProducts = () => {
    // Show all products on the shop page, but still allow filtering
    ...
    if (activeSizes.length > 0) {
      // Note: products.ts currently doesn't have a sizes array in the Product type,
      // but the UI supports it. For now, we'll keep the logic ready.
      // filtered = filtered.filter(p => p.sizes?.some(s => activeSizes.includes(s)));
    }

    // Filter by colors
    if (activeColors.length > 0) {
      // Similar to sizes, we'd need color mapping
    }
  ...
  const FilterSidebar = () => (
    <div className="space-y-10">
      {/* Category Filter */}
      <div>
        <h3 className="text-sm font-bold uppercase tracking-wider mb-4 border-b border-black pb-2">
          Category
        </h3>
        ...
        <div className="grid grid-cols-3 gap-2">
          {SIZES.map((size) => (
            <button
              ...
              className={`py-2 text-xs font-bold border rounded-md transition-all ${
        ...
        {activeFilterCount > 0 && (
          <div className="flex flex-wrap items-center gap-2 mt-6">
            ...
            {activeSizes.map((size) => (
              <span
                key={size}
                className="bg-neutral-100 text-black text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1"
              >
                Size: {size}{" "}
                <button
                  onClick={() => toggleSize(size)}
            ...
            <button
              ...
              className="text-xs font-bold underline hover:text-neutral-500 transition-colors ml-2"
            >
              Clear All
            </button>
          </div>
          ...
          <div className="flex-1">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-10 md:gap-x-8 md:gap-y-12">
              ...
              {!isLoading && filteredProducts.length === 0 && (
                <div className="col-span-full py-20 text-center">
                  ...
                  <button
                    onClick={clearFilters}
                    className="mt-4 font-bold border-b border-black pb-1 uppercase tracking-widest"
                  >
                    Clear all filters
                  </button>
                </div>
            ...
            {!isLoading && filteredProducts.length > 0 && (
              <div className="mt-16 text-center border-t border-neutral-200 pt-16">
                ...
                <p className="text-neutral-400 text-xs font-medium mt-4">
