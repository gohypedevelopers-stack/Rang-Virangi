...
const SwipeHint = () => (
  <div className="flex flex-col items-center justify-center animate-swipe pointer-events-none drop-shadow-2xl">
    {/* The exact icon provided by the user, scaled up for better visibility */}
    <svg width="48" height="68" viewBox="0 0 34 49" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g>
    ...
    <span className="mt-2 text-[10px] tracking-[0.3em] font-medium text-white/80 uppercase drop-shadow-md">
      Swipe
    </span>
  </div>
);
  ...
  return (
    <section className="w-full h-[100dvh] bg-black relative">
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-none">
        {/* Restrict interactable area to the specific box around the t-shirt */}
        <div className="w-[85%] md:w-[60%] lg:w-[55%] h-[40vh] md:h-[50vh] pointer-events-auto relative">
          <CoinModel />

          {/* Fading Hint Overlay */}
          <div
            className={`absolute inset-0 pointer-events-none flex items-center justify-center transition-opacity duration-1000 ${showHint ? 'opacity-100' : 'opacity-0'}`}
          >
            <div className="mt-48">
              <SwipeHint />
            </div>
      ...
      {/* Shop Now Button anchored at the bottom */}
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-20 pointer-events-none">
        <Link
          href="/shop"
          className="pointer-events-auto block text-white text-lg md:text-xl font-medium tracking-wide hover:opacity-90 transition-all pb-2 relative group"
        >
          Shop now
          <span className="absolute bottom-0 left-[5%] w-[90%] h-[2px] bg-white rounded-full transition-all duration-500 ease-in-out group-hover:w-[40%] group-hover:left-[30%]"></span>
        </Link>
      </div>
    </section>
  );
